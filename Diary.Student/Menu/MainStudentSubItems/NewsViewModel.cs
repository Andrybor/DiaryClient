...
{
    [Plugin("News", DPlugin.News, DPlugin.MainStudent, 5, PackIconKind.Web)]
    public class NewsViewModel : DScreen
    {
        ...
        private async void InitNews()
        {
            ...
            if (news != null)
            {
                var result = await SimpleService.Post(Controller.AccountType, Method.Get, TypeAccount.Student.ToString());
                if (result != null)
